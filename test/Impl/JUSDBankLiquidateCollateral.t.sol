...
contract JUSDBankLiquidateCollateralTest is JUSDBankInitTest {
    ...
    function testSelfLiquidationViaOperator() public {
        mockToken1.transfer(alice, 10e18);
        vm.startPrank(alice);
        mockToken1.approve(address(jusdBank), 10e18);
        jusdBank.deposit(alice, address(mockToken1), 10e18, alice);
        ...
        bytes memory param = abi.encode(swapContract,//approveTarget
            swapContract,//swapTarget
            address(alice),//liquidator
            data//data
        );
        vm.startPrank(alice);
        bytes memory afterParam = abi.encode(
            address(flashLoanLiquidate), param
         );
         cheats.expectRevert("SELF_LIQUIDATION_NOT_ALLOWED");
        jusdBank.liquidate(
            alice, // Liquidated
            address(mockToken1),  //collateral used
            alice, //liquidator
            10e18, //10e18 amount
            afterParam,
            10e18 //expectedPrice
        ...
        vm.stopPrank();
        console.log("Alice balance JUSD before liquidation:", IERC20(jusd).balanceOf(address(alice)));
        console.log("Alice balance USDC before liquidation:", IERC20(USDC).balanceOf(address(alice)));
        vm.startPrank(jim);

        cheats.expectRevert("LIQUIDATED_OPERATOR_CAN_NOT_LIQUIDATE");
        jusdBank.liquidate(
            alice, // Liquidated
            address(mockToken1),  // collateral used
            jim, // liquidator Jim
            10e18, //10e18 amount
            afterParam,10e18 //expectedPrice
        );
        console.log("");
        console.log("Alice balance JUSD after liquidation: ", IERC20(jusd).balanceOf(address(alice)));
        console.log("Alice balance USDC after liquidation: ", IERC20(USDC).balanceOf(address(alice)));
     }
